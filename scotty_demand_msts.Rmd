---
title: "Hourly Demand Forecasting of Scotty Ride-Sharing Service"
...
Github : [Hourly Demand Forecasting of Scotty Ride-sharing Service](https://github.com/gabrielerichsonmrp/demandforecasting)
...
my_viridis_color <- get_scale_color(get_pal(my_viridis))
...
maka dapat dilakukan imputasi data dengan asumsi bahwa pada waktu tersebut memang tidak terdapat transaksi, sehingga nilai **demand = 0**.
...
# recipes: square root transformation, center, scale
...
objek time-series yang kita buat memiliki efek seasonal, sehingga seharusnya hanya metode yang dapat mengatasi efek seasonal yang kita gunakan, namun mari kita coba beberapa model dan diterapkan pada setiap objek time-series yang sudah dibuat. Metode yang akan digunakan yaitu:<br>
...
scotty_best_model %>% select(src_sub_area,data_fun_name, model_name,mae_train,mae_test)
```
...
result_submit <- next_week_prediction_result %>%
 ...
 wd <-  as.character(getwd())
 write_csv(result_submit,paste(paste(wd,"/data_result/",sep = ""),"gehm_submit-28032020-001_exp_log.csv",sep=""))
...
ggplotly(ggAcf(residuals(next_week_prediction$fitted[[2]]), lag.max = 504)+
  ggtitle("sxk97 : ACF of residuals"))

ggplotly(gghistogram(residuals(next_week_prediction$fitted[[2]]))+
           labs(title="sxk97 : Histogram of residuals",x="residuals"))
```



### Sub-Area sxk9e
**Residual from TBATS method applied for Sub-Area sxk9e**<br>
...
#par(mfrow=c(1,2), cex=1.1)
ggplotly(ggAcf(residuals(next_week_prediction$fitted[[2]]), lag.max = 504)+
  ggtitle("sxk9e : ACF of residuals"))

ggplotly(gghistogram(residuals(next_week_prediction$fitted[[2]]))+
           labs(title="sxk9e : Histogram of residuals",x="residuals") 
         )
```

### Sub-Area sxk9s
**Residual from TBATS method applied for Sub-Area sxk9s**<br>
```{r}
...
ggplotly(ggAcf(residuals(next_week_prediction$fitted[[3]]), lag.max = 504)+
  ggtitle("sxk9s : ACF of residuals"))

ggplotly(gghistogram(residuals(next_week_prediction$fitted[[3]]))+
...
Hasil analisa data time-series yang sudah dilakukan menunjukan layanan Ride-sharing service Scotty di sub-area sxk97, sxk9e dan sxk9s memiliki seasonal harian dan mingguan. Selain itu, dari model forecasting menggunakan metode ETS, HoltWinter, SARIMA, STLM dan TBATS yang sudah di-uji, model **TBATS** menggunakan objek **Multiple Seasonal Time Series** menghasilkan nilai error paling kecil dengan nilai **MAE < 10**. Hasil forecasting model **TBATS** untuk 3 Desember 2017 pukul 00:00 sampai 9 Desember 2017 pukul 23:00 menunjukan demand paling tinggi pada ketiga sub-area terjadi pada 8 Desember 2017 pukul 18:00, dimana demand di sxk87 sebanyak 85, demand di sxk9e sebanyak 106 dan demand di sxk9s sebanyak 91.
...
[Hyndman, R.J., & Athanasopoulos, G. (2018) Forecasting: principles and practice, 2nd edition, OTexts: Melbourne, Australia.](https://otexts.com/fpp2/complexseasonality.html)[Diakses 24-27 Maret 2020]

[Herlambang, R. Dimas Baghas. 2019. PURRR-Operly Fitting Multiple Time Series](https://algotech.netlify.com/blog/purrr-operly-fitting-multiple-time-series-model/) [Diakses 24-27 Maret 2020] <br>

[Prabhakaran, S. 2016. Time Series Analysis](http://r-statistics.co/Time-Series-Analysis-With-R.html)[Diakses 25 Maret 2020]

[Prastiwi, Ajeng. 2019. Forecasting Time Series with Multiple Seasonal](https://algotech.netlify.com/blog/multiple-seasonal/)[Diakses 26 Maret 2020]<br>

[Dancho, M. 2019. Forecasting Using Multiple Models](https://cran.rstudio.com/web/packages/sweep/vignettes/SW02_Forecasting_Multiple_Models.html)[Diakses 26 Maret 2020]<br>

[Sievert, C. 2019. Interactive web-based data visualization with R, plotly, and shiny](https://plotly-r.com/animating-views.html)[Diakses 27 Maret 2020]
