...
```{r}
# Read Data Test
scotty_test <- read.csv("data_input/scotty_ts/data-test.csv")
glimpse(scotty_test)
```
...
scotty_test <- scotty_test %>% 
   mutate(
      # Transaksi Per-Jam
      datetime = floor_date(ymd_hms(datetime), unit="hour"),
...
```{r}
scotty_input <- read.csv("data_input/scotty_ts/data-train.csv")
```
...
```{r}
scotty_ts_weekly <- scotty_input %>% 
  filter(src_sub_area=="sxk9s") %>% 
  select(demand) %>% 
  ts(frequency = 24*7)

plot_ts_weekly <- scotty_ts_weekly %>% 
  decompose() %>% 
   autoplot()+ 
   labs(
...
```{r}
scotty_msts_weekly <- scotty_input %>% 
  filter(src_sub_area=="sxk9s") %>% 
  select(demand) %>% 
  msts(seasonal.periods = c(24, 24*7))

plot_msts_weekly <- scotty_msts_weekly %>% 
   mstl() %>% 
   autoplot()+ 
   labs(
...
data_msts <- msts(scotty_input  %>% 
  select(demand), seasonal.periods = c(24, 24*7))
data_msts_decompose <- mstl(data_msts, s.window = 24*7)

as.data.frame(data_msts_decompose) %>% 
  mutate(datetime = scotty_input$datetime) %>% 
  mutate(
  ...
  theme_minimal() +
  labs(
    title = "Seasonal Analysis based Daily and Weekly Demand"
  )
