...
result_submit <- next_week_prediction_result %>%
 ...
 wd <-  as.character(getwd())
 write_csv(result_submit,paste(paste(wd,"/data_result/",sep = ""),"gehm_submit-28032020-001.csv",sep=""))
...
**Assumption Checking: Auto Correlation and Normality residual**

### Sub-Area sxk97
**Residual from TBATS in Sub-Area sxk97**
```{r}
checkresiduals(scotty_final_model$fitted[[1]],plot=TRUE, main="Residual from TBATS in Sub-Area sxk97") 
```

### Sub-Area sxk9e
**Residual from TBATS in Sub-Area sxk9e**
```{r}
checkresiduals(scotty_final_model$fitted[[2]],plot=TRUE, main="Residual from TBATS in Sub-Area sxk9e") 
```

### Sub-Area sxk9s
**Residual from TBATS in Sub-Area sxk9s**
```{r}
checkresiduals(scotty_final_model$fitted[[3]],plot=TRUE,df=NULL, main="Residual from TBATS in Sub-Area sxk9s") 
```


```{r,fig.width=12, fig.asp=0.4, eval=FALSE, include=FALSE}
par(mfrow=c(1,3), cex=1.1)
acf(residuals(scotty_final_model$fitted[[1]]),main="Auto Correlation from TBATS in Sub-Area sxk97")
acf(residuals(scotty_final_model$fitted[[2]]),main="Sub-Area sxk9e")
acf(residuals(scotty_final_model$fitted[[3]]),main="Sub-Area sxk9s")

# Box.test(x = residuals(scotty_final_model$fitted[[1]]), type = "Ljung-Box")
# Box.test(x = residuals(scotty_final_model$fitted[[2]]), type = "Ljung-Box")
# Box.test(x = residuals(scotty_final_model$fitted[[3]]), type = "Ljung-Box")
...
# resid(residuals(scotty_final_model$fitted[[1]]))
shapiro.test(x = residuals(scotty_final_model$fitted[[1]]))
shapiro.test(x = residuals(scotty_final_model$fitted[[2]]))
shapiro.test(x = residuals(scotty_final_model$fitted[[3]]))
# 
# library(DMwR)
# 
# accmeasures1=regr.eval(test_series, forecast1$pred)
# 
# scotty_final_model$fitted[[1]]$fitted.values
# 
# 
# 
# assmp <- scotty_model_eval %>%
#   group_by(src_sub_area) %>%
#   filter(mae_test == min(mae_test)) %>%
#   ungroup()
# 
# regr.eval(test_series, forecast1$pred)
