...
Reference:
https://algotech.netlify.com/blog/multiple-seasonal/
...
head(scotty_data)
>>>>>>> 7533a745ed2751bcacbc93ddd323cb7ed62607b8
...
```{r}
scotty_data <- scotty_data %>% 
  gather(src_sub_area, demand, -datetime, -data_sample)
>>>>>>> 7533a745ed2751bcacbc93ddd323cb7ed62607b8
head(scotty_data)
```
...
=======
  ...
  auto.arima = function(x) auto.arima(x, seasonal = TRUE),
...
=======
models <- scotty_ts_model
#scotty_ts_model <- saveRDS(models, "scotty_ts_model.RDS")
wd <-  as.character(getwd())
saveRDS(object=models, file=paste(paste(wd,"/scotty_ts_model/",sep = ""),"scotty_ts_model.rds",sep=""))
...
# 
# wd <-  as.character(getwd())
# write_csv(result_submit,paste(paste(wd,"/data_result/",sep = ""),"submit-28032020-001.csv",sep=""))
...
# Conclusion
```{r}
# conc <- scotty_model_eval %>%
#   group_by(src_sub_area) %>%
#   filter(mae_test == min(mae_test)) %>%
#   ungroup()

checkresiduals(scotty_final_model$fitted[[1]])
checkresiduals(scotty_final_model$fitted[[2]])
checkresiduals(scotty_final_model$fitted[[3]])

acf(residuals(scotty_final_model$fitted[[1]]))
acf(residuals(scotty_final_model$fitted[[2]]))
acf(residuals(scotty_final_model$fitted[[3]]))
...
scotty_model_eval <- scotty_ts_model %>% 
  mutate(
    MAE_error_test = map(fitted, ~ forecast(.x, h = test_size)) %>%
                     map2_dbl(test, ~ mae_vec(truth = reverse(.y$demand,rec,src_sub_area), 
                                              estimate = reverse(.x$mean,rec,src_sub_area))),
    MAE_error_train = map(fitted, ~ forecast(.x, h = test_size)) %>%
                      map2_dbl(train, ~ mae_vec(truth = reverse(.y$demand,rec,src_sub_area), 
                                                estimate = reverse(.x$fitted,rec,src_sub_area)))
    ) %>% 
  arrange(src_sub_area, MAE_error_test,MAE_error_train) 
